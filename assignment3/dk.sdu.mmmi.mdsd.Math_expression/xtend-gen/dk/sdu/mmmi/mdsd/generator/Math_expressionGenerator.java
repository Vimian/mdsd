...
public class Math_expressionGenerator extends AbstractGenerator {
  ...
  public void doGenerate(final Resource resource, final IFileSystemAccess2 fsa, final IGeneratorContext context) {
    throw new Error("Unresolved compilation problems:"
      ...
      + "\nAmbiguous feature call.\nThe methods\n\tgenerateFile(String, CharSequence) in IFileSystemAccess and\n\tgenerateFile(String, InputStream) in IFileSystemAccessExtension3\nboth match.");
  }

  public abstract Object compile(final MathExp math);
  ...
  public static Map<String, Integer> compute(final MathExp math) {
    Map<String, Integer> _xblockexpression = null;
    {
      ...
      Math_expressionGenerator.variables = _hashMap;
      EList<VarBinding> _variables = math.getVariables();
      for (final VarBinding varBinding : _variables) {
        Math_expressionGenerator.computeExpression(varBinding);
      }
      _xblockexpression = Math_expressionGenerator.variables;
    }
    return _xblockexpression;
  }

  protected static int _computeExpression(final VarBinding binding) {
    Math_expressionGenerator.variables.put(binding.getName(), Integer.valueOf(Math_expressionGenerator.computeExpression(binding.getExpression())));
    return (Math_expressionGenerator.variables.get(binding.getName())).intValue();
  }
  ...
  protected static int _computeExpression(final Plus exp) {
    int _computeExpression = Math_expressionGenerator.computeExpression(exp.getLeft());
    int _computeExpression_1 = Math_expressionGenerator.computeExpression(exp.getRight());
  ...
  protected static int _computeExpression(final LetBinding exp) {
    return Math_expressionGenerator.computeExpression(exp.getBody());
  }

  protected static int _computeExpression(final VariableUse exp) {
    return Math_expressionGenerator.computeBinding(exp.getRef());
  }

  protected static int _computeBinding(final VarBinding binding) {
    Integer _xblockexpression = null;
    {
      boolean _containsKey = Math_expressionGenerator.variables.containsKey(binding.getName());
      boolean _not = (!_containsKey);
      if (_not) {
        Math_expressionGenerator.computeExpression(binding);
      }
      _xblockexpression = Math_expressionGenerator.variables.get(binding.getName());
    }
    return (_xblockexpression).intValue();
  }

  protected static int _computeBinding(final LetBinding binding) {
    return Math_expressionGenerator.computeExpression(binding.getBinding());
  }
